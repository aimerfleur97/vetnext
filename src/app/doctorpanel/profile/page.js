...
const Profile = () => {
    ...
    const getDoctorDetails = async () => {
        try {
            setIsLoading(true)
            const doctorDetails = JSON.parse(localStorage.getItem("doctor"))
            const response = await doctorDetail(doctorDetails._id)
    ...
    const formik = useFormik({
        ...
        onSubmit: async (values, { setSubmitting }) => {
            try {
                setIsLoading(true)
                const doctorDetails = JSON.parse(localStorage.getItem("doctor"))
                const response = await doctorUpdate(doctorDetails._id, values)
